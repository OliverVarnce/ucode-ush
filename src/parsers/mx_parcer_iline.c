...
void mx_parser_err(t_envp *var, char c, int flag) {
    if (flag == 1) {
        mx_printerror("ush: command substitution is not supported\n");
        mx_printerror("     Do not use symbols ` and ()\n");
        mx_envp_replace(&var, "?=1");
    }
    if (flag == 2) {
        mx_printerror("ush: multiline command input is not supported\n");
        mx_printerror("     Use pair symbols in pair\n");
        mx_envp_replace(&var, "?=1");
    }
    if(c)
    return;
